...
// = TSA_Initialize
// Description: TSA接口清理
TSA_EXPORT void TSA_CoInitialize();
...
TSA_EXPORT int TSA_GetCurrentTime( const char * szUrl, std::string * pCurrentTime );
...
// Return: 成功返回0，失败返回错误号
TSA_EXPORT int TS_Parse( const char* szUrl, const char* RespData,  std::string& PraseData ,int flag = 0);
...
//   flag            - [输入参数] 标志 0-时间  1-证书
// Return: 成功返回0，失败返回错误号
TSA_EXPORT int TS_Parse(const char* RespData,  std::string& PraseData , int flag = 0);
