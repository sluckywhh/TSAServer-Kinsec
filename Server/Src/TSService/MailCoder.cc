...
//Base64编码
int CMailCoder::base64_encode(const char *pSrc, unsigned int nSize, char *pDest)
{
...
int CMailCoder::base64_decode(const char *pSrc, unsigned int nSize, char *pDest)
{
...
//UUCode编码
int CMailCoder::UU_encode(const char *pSrc, unsigned int nSize, char *pDest)
{
	if ((pSrc == NULL) || (pDest == NULL) || (nSize <= 0)) return 0;
...
//UUCode解码
int CMailCoder::UU_decode(const char *pSrc, unsigned int nSize, char *pDest)
{
	char C[4]; //临时阅读变量
	char Tmp;
	int CurrIndex, Index;
...
//Quoted编码
int CMailCoder::quoted_encode(const char *pSrc, unsigned int nSize, char *pDest)
{
	unsigned int Len;
...
//Quoted解码
int CMailCoder::quoted_decode(const char *pSrc, unsigned int nSize, char *pDest)
{
	if ((pSrc == NULL) || (pDest == NULL) || (nSize <= 0)) return 0;
	...
	const char *InLimitPtr= pSrc + nSize;
	char *pDestOrg = pDest;
