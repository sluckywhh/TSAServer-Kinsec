...
#define PID_FILE   "Pid/TSAService.pid"
CTSAServiceApp g_app;

void OnBeforeExit( int signal )
{
	remove( PID_FILE );
	printf( "[Info] 时间戳颁发服务停止成功！\n" );
	exit( 0 );
}
...
void Service()
{
	...
	{
		printf( "[Error] 启动时间戳颁发服务出错：创建PID文件失败！\n" );
		exit( 0 );
	}
...
void Start()
{
	...
	if( fp )
	{
		...
		{
			printf( "[Info] 时间戳颁发服务已在运行，无法再次启动！\n" );
			return ;
		}
	}
	Service();
...
int main( int argc, char * argv[] )
{
	...
	printf( " Usage   : ./TSAService start|stop|restart\n" );
	return 0;
}
