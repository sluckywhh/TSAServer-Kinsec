...
int KSSIGN_GetTimeStampInfo(
	unsigned char *pbIn,				// 输入数据
	int nInLen,							// 输入数据的长度
	int nType,							// 类型：1－原文（TS_TST_INFO），2－时间戳证书，3－签名值，4－签名算法，5－原文（TS_TST_INFO）摘要，6－摘要算法，7－版本，8－时间戳时间，9－初始原文的摘要算法，10－初始原文的摘要，11－时间戳序列号
	unsigned char *pbOut,				// 输出数据
	int *pnOutLen);						// 输出数据的长度
